...
import { ASSET_MAP, type Asset } from '@/lib/data/assets'
import {
  ALLOCATIONS_ARS,
  ALLOCATIONS_USD,
  type AllocationSlot,
} from '@/lib/data/portfolio-allocations'
...
// Assets considered high-risk for short horizons
...
function adjustForHorizon(slots: AllocationSlot[], currency: 'ARS' | 'USD'): AllocationSlot[] {
  ...
  const adjusted = slots.map((slot) => {
    if (HIGH_RISK_IDS.has(slot.assetId)) {
      const reduced = slot.weight * 0.5 // reduce by 50% (within the 30-60% band)
...
export interface CategoryWeight {
  name: string
  weight: number // raw percent sum
  pct: number // normalized percentage
}
